...
<body>
	...
	<form name="frm_view">
		...
		<table>
			...
			<tr>
				<th>제목</th>
				<td colspan="5" id="view_title">${boardFileVo.title}</td>
			</tr>
			<tr>
				<th>내용</th>
			...
			<tr>
				<th>첨부파일</th>
				<td colspan="5"><a
					href="download.jsp?oName=${boardFileVo.originFile}&sName=${boardFileVo.saveFile}">
						<img src="../uploads/${boardFileVo.saveFile}" width="200px"
						height="150px"><br> ${boardFileVo.originFile}
				</a></td>
			</tr>
		</table>
	</form>
	...
	<div id="view_btn">
		<c:if
			test="${(!empty member_info) and ((member_info.member_idx eq boardFileVo.member_idx) or (member_info.grade eq 1) or (member_info.grade eq 2))}">
		...
		<c:if
			test="${(!empty member_info) and ((member_info.member_idx eq boardFileVo.member_idx) or (member_info.grade eq 1) or (member_info.grade eq 2))}">
			<input type="button"
				onclick="location.href='edit.jsp?no=${boardFileVo.board_idx}'"
